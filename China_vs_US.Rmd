...
p1 <- ggplot(df_stats, aes(x = Year, y = Mean, colour = Country))+
  geom_line(data = df_lite %>% filter(Country == "United States"), aes(x=Year, y=Articles, group = Cancer), linewidth = 0.35, linetype = "dashed")+
  geom_line(data = df_lite %>% filter(Country == "China"), aes(x=Year, y=Articles, group = Cancer), linewidth = 0.35, linetype = "dashed")+
  geom_line(linewidth = 1.2)+
  scale_y_continuous(limits = c(0, max((df_lite %>% filter(Country == "China"))[,"Articles"])*1.1), expand = c(0,0))+
  scale_x_continuous(limits = c(1990, 2024), breaks = seq(1990, 2024, 2), expand = c(0.002,0.002))+
  ...
  theme(
    ...
    text = element_text(family = "Calibri", size = font_size+4),
    axis.title = element_text(size = font_size + 6),
...
```{r}
p2 <- ggplot(df_stats, aes(x = Year, y = Mean, colour = Country))+
  geom_line(linewidth = 0.5)+
  scale_y_continuous(limits = c(0, max((df_stats %>% filter(Country == "China"))[,"Mean"])*1.1), expand = c(0,0))+
  scale_x_continuous(limits = c(1990, 2024), breaks = seq(1990, 2024, 2), expand = c(0.002,0.002))+
  scale_color_manual(values = c("China" = "#A000C2",
                                "United States" = "#166128"))+
  theme_minimal(base_family = "Calibri")+
  ylab("Number of articles")+
  labs(title = "Evolution of published articles about top 5 studied cancers in the United States and China")+
  theme(
    plot.background = element_rect(fill = '#f6f8fa'),
...
pie1990 <- ggplot(df_compiled_disc, aes(x="", y=Y1990, fill=Country))+
  ...
  geom_text(data = subset(df_compiled_disc, Country == "China"), aes(x=1.9, y=label_position_1990, label=Y1990_text), inherit.aes = FALSE, hjust=0.5, vjust=1, family = "Calibri", size = font_size-10)+
  ...
  theme(
    ...
    text=element_blank(),
    plot.title = element_text(family = "Calibri", size = font_size + 7, hjust = 0.5, margin = margin(t = 21, r = 0, b = 0, l = 0)),
...
pie2024 <- ggplot(df_compiled_disc, aes(x="", y=Y2024, fill=Country))+
  geom_bar(stat="identity", width=1, color="white") +
  coord_polar("y", start=0)+
  geom_text(data = subset(df_compiled_disc, Country == "China"), aes(x=2, y=label_position_2024, label=Y2024_text), inherit.aes = FALSE, hjust=0.5, vjust=1, family = "Calibri", size = font_size-10)+
...
full_plot <- p1 + inset_element(pies, left = 0, bottom = 0.35, right = 0.55, top = 1.05) & theme(plot.background = element_rect(linewidth = NA))
...
full_plot_lite <- p2 + inset_element(pies, left = 0, bottom = 0.35, right = 0.55, top = 1.05) & theme(plot.background = element_rect(linewidth = NA))

full_plot_lite

ggsave(plot = full_plot_lite, filename = file.path(out_dir, "evolution_lines_pies_only_mean.jpg"), device = "jpg", width = width_out, height = height_out, units = "px")
```
